...
版本 2.2.0 起会根据车辆上报的能力创建以下控制实体：

- **车辆电源锁**：解锁代表启动车辆，上锁代表关闭车辆
- **车辆设防**：开启或关闭设防状态
- **开启座桶**：发送一次开启座桶指令
...
“车辆电源锁”是为了兼容 Apple Home 的门锁界面而提供的逻辑映射，并不代表车辆机械锁的真实状态。HomeKit 中的锁需要使用 `accessory` 模式单独桥接。
...
### 版本 2.2.1
- **改进**: 门锁操作立即显示正在上锁或解锁
- **改进**: 云端确认后先采用目标状态，再在后台与车辆真实状态校准
- **改进**: 移除控制路径中的固定两秒等待

### 版本 2.2.0
- **变更**: 将整车电源改为门锁实体，解锁启动车辆、上锁关闭车辆
- **改进**: 可作为 DoorLock 配件单独桥接到 Apple Home
